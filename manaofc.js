...
socket.ev.on("messages.upsert", async ({ messages }) => {
    ...
    try {
        ...
        const mode = userConfig.MODE || "public";
        const owner = userConfig.OWNER_NUMBER;
        ...
        if (!isOwner) {
            if (commandCooldowns.has(sender)) {
                ...
                if (diff < COMMAND_COOLDOWN) {
                    return reply(
                        `⏳ Please wait ${(
                            (COMMAND_COOLDOWN - diff) /
                            1000
                        ).toFixed(1)}s before using another command.`
                    );
                }
            }
        ...
        // ================= OWNER ONLY =================
        if (cmdObj.fromMe && !isOwner) {
            return reply("❌ This command is owner only.");
        }

        // ================= GROUP ONLY =================
        if (cmdObj.groupOnly && !isGroup) {
        ...
        // ================= EXECUTE COMMAND =================
        await cmdObj.function(socket, mek, mek, {
            from,
            sender,
            isOwner,
            isGroup,
...
setInterval(() => {
    ...
    for (const [user, time] of commandCooldowns) {
        if (now - time > COMMAND_COOLDOWN * 5) {
